...
- **Cross-sectional reversal research** lives in **`~/reversal-research`** (standalone git repo): Yahoo OHLC backtests, H1–H4, H2b, **M1** (~21d loser momentum), EW + buy-hold baselines; working notes **`docs/CURRENT_KNOWLEDGE.md`**, roadmap **`docs/NEXT_STEPS.md`**. Specs: **`docs/EXPERIMENT_AGENT_PROMPT.md`** → **`experiments/*.md`** (`python3 scripts/run_experiment_discovery_agent.py`). Implement one spec: **`docs/EXPERIMENT_IMPLEMENTER_PROMPT.md`** + **`python3 scripts/run_experiment_implementer_agent.py experiments/NNN-….md`**. Plots: `run_study.py --plot …` then `telegram-bot/send_photo_now.py` (needs network).
