...
- **Run the bot**: From a terminal outside Cursor, run `python3 telegram-bot/agent_bot.py`. The bot invokes `cursor agent` with `--print --trust --force` so it can run commands without prompting. Session ID is stored in `telegram-bot/.cursor_agent_session` so restarts keep the same conversation. The bot writes `telegram-bot/chat_id` when you message it (used by reminders).
